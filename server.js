...
io.on('connection', (socket) => {
    ...
    // --- NOVA FUNÇÃO DE APAGAR DO CATÁLOGO ---
    socket.on('remover_item_catalogo_global', (data) => {
        const cat = data.catType;
        if (globalCatalog[cat]) {
            globalCatalog[cat] = globalCatalog[cat].filter(it => it.name.toLowerCase() !== data.name.toLowerCase());
            salvarCatalogo();
            console.log(`🗑️ Item removido do catálogo global (${cat}):`, data.name);
            socket.broadcast.emit('item_removido_catalogo_global', data);
        }
    });
